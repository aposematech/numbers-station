...
def get_cipher_text(alpha, text, key):
    return "".join(
        str((ord(a) - ord(b)) % len(alpha)).zfill(2) for a, b in zip(text, key)
    )


def get_cipher_text_blocks(text, size):
    return " ".join(
        [text[i : i + size].ljust(size, "0") for i in range(0, len(text), size)]
    )
...
def handler(event, context):
    ...
    plain_text = get_alpha_only(
        parameters.get_parameter(
            name=os.environ["SECRET_TRANSMISSION_NAME"], decrypt=True
        )
    )
    one_time_pad = get_one_time_pad(alphabet, plain_text)
    cipher_text = get_cipher_text_blocks(
        get_cipher_text(alphabet, plain_text, one_time_pad), 5
    )
    # create qrcode
    colors = ["red", "white"]
    ...
    return {
        ...
        "body": json.dumps({"one_time_pad": one_time_pad}),
    }
