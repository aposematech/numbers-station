...
resource "aws_s3_bucket" "website_bucket" {
  bucket = "${var.subdomain_name}.${var.registered_domain_name}"
}
...
resource "aws_cloudfront_distribution" "cloudfront_distribution" {
  origin {
    ...
    origin_id   = aws_acm_certificate.certificate.domain_name
  }

  enabled             = true
  default_root_object = var.default_page

  aliases = [aws_acm_certificate.certificate.domain_name]

  default_cache_behavior {
    allowed_methods  = ["GET", "HEAD"]
    cached_methods   = ["GET", "HEAD"]
    target_origin_id = aws_acm_certificate.certificate.domain_name
